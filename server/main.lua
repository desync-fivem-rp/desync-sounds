...
-- RegisterNetEvent('InteractSound_SV:PlayWithinDistanceAI')
-- AddEventHandler('InteractSound_SV:PlayWithinDistanceAI', function(maxDistance, soundFile, soundVolume)
--     local src = source
--     local DistanceLimit = 300

--     local savedFilename = soundFile .. ".ogg"

--     if maxDistance < DistanceLimit then

-- 	TriggerClientEvent('InteractSound_CL:PlayWithinDistanceAI', -1, GetEntityCoords(GetPlayerPed(src)), maxDistance, soundFile, soundVolume)
--     else
--         print(('[interact-sound] [^3WARNING^7] %s attempted to trigger InteractSound_SV:PlayWithinDistance over the distance limit ' .. DistanceLimit):format(GetPlayerName(src)))
--     end
-- end)
...
-- function SaveAudioToFile(response, filePath)
--     local file = io.open(filePath, "wb")  -- Open the file for writing in binary mode
--     if file then
--         file:write(response)  -- Write the response data to the file
